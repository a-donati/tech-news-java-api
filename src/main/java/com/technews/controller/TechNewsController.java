...
//post/put dashboard - allow to post from dashboard
@PostMapping("/posts")
...
//updated post - refresh dashboard
...
//if comment area is blank - render message
...
//            if session is valid, save comment to user
            if (request.getSession(false) != null) {
                User sessionUser = (User) request.getSession().getAttribute("SESSION_USER");
                comment.setUserId(sessionUser.getId());
                commentRepository.save(comment);
//                return the user to edit-post.html
                return "redirect:/post/" + comment.getPostId();
            } else {
//                if session invalid, return to login page
                return "login";
            }
        }
    }
...
//    if the session is valid, vote UserId === sessionUser Id - save vote
        if (request.getSession(false) != null) {
